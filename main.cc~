...
//cout<<mean<<endl<<var<<endl;
    ...
    for (int i = 0; i < ensemble; i++)
	{
		...
		for(int j=0;j<(int(floor(runtime/dt)));j++)
		{
			...
			double thetanoise=dt*Dr*Dr/tan(theta)+sqrt(2.0)*Dr*sqdt*norm_rand();
			double phinoise=sqrt(2.0)*Dr*sqdt*norm_rand()/sin(theta);
...
for(int j=0;j<(int(floor(runtime/dt)));j++)
{
	cout<<dt*j<<" " <<(nx[j]*nx[0])/(ensemble*ensemble)<<" "<<(ny[j]*ny[0])/(ensemble*ensemble)<<" "<<(nz[j]*nz[0])/(ensemble*ensemble)<<endl;
